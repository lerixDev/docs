...
Atelerix delivers push notifications on iOS using Apple Push Notification service (APNs) directly — no Firebase required. After you complete the [iOS installation](/frameworks/ios/installation), follow the steps below to enable push delivery in your app.
...
public struct LerixNotificationPayload {
    public let notificationId: String? // this notification's id in Atelerix
...
|---|---|---|
| `getRegisteredTokenId()` | Atelerix's own id for this registered device, assigned once `requestPermissions()` succeeds and the token is registered with the backend | The value to put in **device tokens** when sending a notification to this specific device — from the dashboard's **Send notification** tab, or the [REST API](/api-reference/push-notifications) |
| `getDeviceToken()` | The raw APNs device token (hex-encoded) | Rarely needed directly; useful for diagnostics or if you're calling APNs yourself outside of Atelerix |
| `getDeviceId()` | The device's `UIDevice.identifierForVendor` — a local iOS system identifier, unrelated to Atelerix's backend | Local diagnostics only; **not** what the dashboard's send flow expects |
...
- The file must already ship inside your app's bundle — Atelerix only passes the filename through, it never hosts or uploads sound files.
