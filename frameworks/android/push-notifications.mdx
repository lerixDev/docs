...
Atelerix delivers push notifications on Android using Firebase Cloud Messaging (FCM) — but unlike a typical FCM integration, **you don't add a `google-services.json` file or the Google Services Gradle plugin to your app**. The SDK registers your device against Atelerix's own Firebase project internally, using this Atelerix project's Sender ID (fetched from the backend automatically). After you complete the [Android installation](/frameworks/android/installation), follow the steps below to enable push delivery.
...
3. Upload the downloaded JSON file to the Atelerix dashboard, under **Notifications → Settings**.
...
data class LerixNotificationPayload(
    val notificationId: String?, // this notification's id in Atelerix
    val title: String?,
    val body: String?,
...
| `getDeviceToken()` | The raw FCM registration token | Rarely needed directly; useful for diagnostics or if you're calling FCM yourself outside of Atelerix |
| `getDeviceId()` | `Settings.Secure.ANDROID_ID` — a local Android system identifier, unrelated to Atelerix's backend | Local diagnostics only; **not** what the dashboard's send flow expects |
...
- The file must already ship inside your app under `res/raw/` — Atelerix only passes the filename through, it never hosts or uploads sound files.
