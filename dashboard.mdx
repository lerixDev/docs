...
For backend projects, every request your service handled — its route,
duration, status, the database queries and outbound calls it made — with
per-route counts and latency percentiles, the busiest client addresses, and
a waterfall for each kept request. Open an issue and the request that
produced it is shown inline. The client-address policy (masked, hashed or
full) lives under **Project settings → Tracing**. See
[Request Tracing](/modules/request-tracing).
