...
يُرسِل Atelerix الإشعارات على Android باستخدام Firebase Cloud Messaging (FCM) — لكن خلافاً لتكامل FCM المعتاد، **لن تحتاج لإضافة ملف `google-services.json` أو إضافة Google Services Gradle plugin إلى تطبيقك**. يسجّل الـ SDK جهازك داخلياً ضمن مشروع Firebase الخاص بـ Atelerix، باستخدام معرّف Sender ID الخاص بمشروعك في Atelerix (يُجلَب تلقائياً من الباك إند). بعد إتمام [تثبيت Android](/ar/frameworks/android/installation)، اتّبع الخطوات أدناه لتفعيل تسليم الإشعارات.
...
3. ارفع ملف JSON الذي تم تنزيله إلى لوحة تحكم Atelerix، ضمن **الإشعارات ← الإعدادات**.
...
data class LerixNotificationPayload(
    val notificationId: String?, // معرّف هذا الإشعار في Atelerix
    val title: String?,
    val body: String?,
...
| `getDeviceToken()` | رمز تسجيل FCM الأصلي | نادراً ما تحتاجه مباشرة؛ مفيد للتشخيص أو إذا كنت تستدعي FCM بنفسك خارج Atelerix |
| `getDeviceId()` | معرّف `Settings.Secure.ANDROID_ID` — معرّف نظام Android محلي، لا علاقة له بالباك إند الخاص بـ Atelerix | للتشخيص المحلي فقط؛ **ليس** ما تتوقّعه عملية الإرسال من لوحة التحكم |
...
- يجب أن يكون الملف موجوداً بالفعل داخل تطبيقك تحت `res/raw/` — يمرّر Atelerix اسم الملف فقط، ولا يستضيف أو يرفع ملفات الصوت.
