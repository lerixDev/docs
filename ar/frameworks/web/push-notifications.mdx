...
description: "إشعارات المتصفح لـ Lerix — Web Push القياسي (VAPID)، بدون Firebase، بدون OneSignal."
---

## مقدمة

يُرسِل Lerix إشعارات الويب باستخدام **بروتوكول Web Push القياسي (VAPID)** —
نفس المعيار المفتوح الذي تطبّقه Chrome وFirefox وEdge وSafari الحديث أصلياً.
بدون مشروع Firebase، بدون حساب OneSignal — يُنشئ Lerix زوج المفاتيح
ويديره نيابةً عنك.

<Note>
  هذه حزمة منفصلة عن SDK الخاص بـ Flutter — `@lerix-dev/lerix-js` مكتبة
  ...
  Flutter — بما في ذلك Flutter Web — لا تستخدم هذه الحزمة؛ حزمة `lerix`
...
`node_modules/@lerix-dev/lerix-js/sw/lerix-sw.js` إلى جذر موقعك
العام بحيث يكون متاحاً على `https://yoursite.com/lerix-sw.js` (أي مسار
...
| الدالة | الوصف |
|---|---|
| `Lerix.init(config)` | يهيّئ الـ SDK. استدعِها مرة واحدة. |
| `Lerix.notifications.init()` | يسجّل هذا التطبيق/المستخدم لدى الباك إند. استدعِها قبل أي دالة أخرى على `notifications`. |
...
يستقبل كلا الاستدعائين كائناً من نوع `LerixNotificationPayload` — بنفس
...
`lerix` الخاصة بـ Flutter تدعم iOS وAndroid والويب أصلياً من حزمة واحدة،
وتتواصل مباشرةً مع Push API الخاصة بالمتصفح بدون أي اعتماد JS منفصل إطلاقاً.
نفس استدعاءات `Lerix.notifications.*`، ونفس شكل `LerixNotificationPayload`،
على كل منصة:
