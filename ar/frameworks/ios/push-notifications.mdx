...
يُرسِل Atelerix الإشعارات على iOS باستخدام Apple Push Notification service (APNs) مباشرةً — بدون الحاجة إلى Firebase. بعد إتمام [تثبيت iOS](/ar/frameworks/ios/installation)، اتّبع الخطوات أدناه لتفعيل تسليم الإشعارات في تطبيقك.
...
4. ارفع ملف `.p8` إلى لوحة تحكم Atelerix، ضمن **الإشعارات ← الإعدادات**.
5. في **Certificates, Identifiers & Profiles → Identifiers**، تأكّد من تفعيل خاصية **Push Notifications** على App ID الخاص بتطبيقك. هذا منفصل عن مفتاح `.p8` أعلاه ومطلوب رغم أن مصادقة Atelerix المعتمدة على الرمز (token-based) لا تحتاج شهادة لكل تطبيق — إذ ترفض APNs الإشعارات الموجّهة لـ App ID لا تملك سجلّاً له.
...
public struct LerixNotificationPayload {
    public let notificationId: String? // معرّف هذا الإشعار في Atelerix
...
|---|---|---|
| `getRegisteredTokenId()` | معرّف Atelerix الخاص بهذا الجهاز المُسجَّل، يُخصَّص بمجرد نجاح `requestPermissions()` وتسجيل الرمز لدى الباك إند | القيمة التي تضعها في **رموز الأجهزة** عند إرسال إشعار لهذا الجهاز تحديداً — من تبويب **إرسال إشعار** في لوحة التحكم، أو [REST API](/ar/api-reference/push-notifications) |
| `getDeviceToken()` | رمز جهاز APNs الأصلي (مُرمَّز بصيغة hex) | نادراً ما تحتاجه مباشرة؛ مفيد للتشخيص أو إذا كنت تستدعي APNs بنفسك خارج Atelerix |
| `getDeviceId()` | معرّف `UIDevice.identifierForVendor` الخاص بالجهاز — معرّف نظام iOS محلي، لا علاقة له بالباك إند الخاص بـ Atelerix | للتشخيص المحلي فقط؛ **ليس** ما تتوقّعه عملية الإرسال من لوحة التحكم |
...
- يجب أن يكون الملف موجوداً بالفعل داخل حزمة تطبيقك — يمرّر Atelerix اسم الملف فقط، ولا يستضيف أو يرفع ملفات الصوت.
