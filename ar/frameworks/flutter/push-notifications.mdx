...
description: "أعِدّ الإشعارات لأنظمة Android وiOS وHuawei في Flutter."
---

## مقدمة

يُرسِل Atelerix الإشعارات باستخدام الخدمة الأصلية لكل منصة — Firebase Cloud Messaging (FCM) على Android، وApple Push Notification service (APNs) مباشرةً على iOS، بدون الحاجة إلى Firebase. بعد إتمام [تثبيت Flutter](/ar/frameworks/flutter/installation)، اتّبع خطوات كل منصة أدناه لتفعيل تسليم الإشعارات في تطبيقك.
...
بعد رفع ملف `google-services.json` (لـ Android) و/أو مفتاح `.p8` (لـ iOS) من تبويب **الإشعارات ← الإعدادات** في لوحة تحكم مشروعك، تظهر شارة **تم الإعداد (Configured)** على بطاقة تلك المنصة مع بيانات الاعتماد التي تحملها — يمكنك الضغط عليها مجدداً في أي وقت لمراجعتها أو رفع بديل لها.
...
يستقبل كل من `setOnNotificationReceived` و`setOnNotificationTapped` كائناً من نوع `AtelerixNotificationPayload` — بنفس الشكل على iOS وAndroid، بغضّ النظر عن اختلاف طريقة تسليم كل منصة للإشعار الأصلي:
...
| `getDeviceToken()` | رمز الدفع (push token) الأصلي للمنصة — رمز جهاز APNs على iOS، ورمز تسجيل FCM على Android | نادراً ما تحتاجه مباشرة؛ مفيد للتشخيص أو إذا كنت تستدعي APNs/FCM بنفسك خارج Atelerix |
